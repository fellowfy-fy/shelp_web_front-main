...
import {
	FormControl, Tag, InputGroup, InputLeftElement,
...
const CollectionPage = (props) => {
	...
	const data = {
		...
		title: "Trumpet/Mermaid Scoop Ankle-Length Lace Tulle Mother of the Bride Dress With Sequins Beading  - JJ's House",
		description: "Product Code: #217292 Fabric: Lace, Tulle Embellishment: Beading, Sequins Silhouette: Trumpet/Mermaid Length: Ankle-Length Neckline: Scoop Straps & Sleeves: Short Sleeve Fully Lined: Yes Built-In Bra: Yes Boning: No Features: No Leg Slit Back Style: Back Zip, Scoop",
		isNew: true,
		imageURL:
			'https://i.pinimg.com/564x/ba/37/0e/ba370efb59365508e2b8d49f7575b105.jpg',
		...
		<Container maxW={"100%"} px={0}>
			...
			<Box pb={10} >
				<Container maxW={"container.xl"}>
					...
					<HStack alignContent={'center'}  justifyContent={'space-between'} gap={6}>
						<VStack textAlign='center' gap={3} alignItems='flex-start' width={'100%'} maxW={'800px'}>
							<Box textStyle={'collectionTitle'}>{data.collection.tilte}</Box>
							<Text fontSize='md'>
								Paystack helps businesses in Africa get paid by anyone, anywhere in the
								world
							</Text>
							<HStack fontSize='17px' gap={5} justifyContent='flex-start' fontWeight={500}>
								<HStack fontSize={'15px'} fontWeight={600} gap={2}>
									{data.collection.authorUsername ? (
										<Link to={`/${data.collection.authorUsername}`}>
											<Avatar src={data.collection.authorPoriflePic} alt='user profile pic' size={"sm"} />
										</Link>
									) : (
									...
									{data.collection.authorUsername ? (
										<Link fontSize='15px' fontWeight={600} to={`/${data.collection.authorUsername}`}>{data.collection.authorUsername}</Link>
									) : (
										<Skeleton w={"100px"} h={"10px"} />
									)}
								</HStack>
								<Divider color={'#FE5F75'} height={'20px'} border={'1px'} orientation='vertical' />
								<HStack gap={2}><Box fontWeight={600}>12</Box> <Box> products</Box></HStack>
								<Divider color={'#FE5F75'} height={'20px'} border={'1px'} orientation='vertical' />
								<HStack gap={2}><Box fontWeight={600}>12</Box> <Box> posts</Box></HStack>
							</HStack>						
						</VStack>
						<VStack gap={3} alignItems={'flex-end'} flexWrap='wrap' justify={'flex-start'} cursor={"pointer"}>
							<Button size={"sm"} leftIcon={<HStack gap={1}><Box>12</Box><BiHeart /> </HStack>} colorScheme='blue' variant='outline'>
								{data.collection.isLiked ? "Unlike" : "Like"}
							</Button>
							<Button size={"sm"} leftIcon={<HStack gap={1}><Box>12</Box><HiOutlineUsers /> </HStack>} colorScheme='blue' variant='outline'>
								{isFollowing ? "Unfollow" : "Follow"}
							</Button>
							<Button size={"sm"} rightIcon={<FaRegShareSquare />} colorScheme='blue' variant='outline'>
								Share
							</Button>
						</VStack>
					</HStack>
...
*/}
				...
				<FeedPosts posts={collection_posts} items={collectionItems} observer={observerTarget} postData={data} />
				<Center>
					<Button border={'1px'} px={20} borderRadius={20} background={'transparent'} borderColor={'black'} _hover={{ background: 'black', color: 'white' }}>Load more</Button>
				</Center>
			</Container>
		</Container>
	);
